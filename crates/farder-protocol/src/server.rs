...
pub enum ServerRequest {
    ...
    /// Fetch the device-lifecycle events for one identity — the raw signed
    /// `Event` bytes carrying `DeviceAuthorized` (one `DeviceCert` each) and
    /// `DeviceRevoked` (one revoked `DeviceId` each), oldest-first. A device
    /// cert is the log-valid trust anchor for the receive-side leaf-binding
    /// gate (`verify_leaf_binding`), so it must come from HERE (the log) —
    /// NEVER from the commit being validated. A client tells the two payloads
    /// apart by decoding each event's payload enum — no new wire shape. Public
...
pub enum ServerResponse {
    ...
    /// Answer to `FetchDeviceCerts`: raw signed `Event` bytes, oldest-first,
    /// mixing `DeviceAuthorized` (one `DeviceCert` each) and `DeviceRevoked`
