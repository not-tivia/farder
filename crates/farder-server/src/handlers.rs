...
pub fn handle_request(
    ...
    match request {
        ...
        ServerRequest::StartVoice { channel_id } => {
            ...
            let channel = channels::get_channel(conn, channel_id)?
                .ok_or_else(|| anyhow::anyhow!("channel not found"))?;
            // Voice channels and DMs are valid; reject text/announcement/category/thread.
            if channel.channel_type != ChannelType::Voice
            ...
            // Signal voice state mutation via a dedicated EventTarget; the dispatcher
            // (async) will call voice::start_transmit. handle_request is sync, so we
            // can't touch the tokio RwLocks directly here.
            let mut events: Vec<BroadcastEvent> = vec![BroadcastEvent {
            ...
            // For DMs, ring the other participant if we're the first transmitter.
            if channel.channel_type == ChannelType::Dm {
                let active = channels::get_voice_participants(conn, channel_id)?;
                let others_active = active.iter().filter(|(pk, _)| pk != member).count();
                if others_active == 0 {
                    if let Some((_, other_pk)) = channels::list_dm_channels(conn, member)?
                        .into_iter()
                        .find(|(ch, _)| ch.id == channel_id)
                    {
                        let display_name = members::get_member(conn, member)?
...
mod tests {
    ...
    #[test]
    fn test_start_voice_in_dm_emits_ringing() {
        let (conn, owner) = setup();
        let other = add_member(&conn, "Other");
        let ch_id = channels::create_dm_channel(&conn, &owner, &other).unwrap();
        let result = handle_request(
            &conn,
            &owner,
            true,
        ...
        .unwrap();
        assert!(matches!(result.response, ServerResponse::Ok));
        // First event signals the voice state mutation; second targets the callee with VoiceCallIncoming.
        let ringing = result.events.iter().find(|e| matches!(
            e.event,
            ServerEvent::VoiceCallIncoming { .. }
        ));
        assert!(ringing.is_some(), "DM StartVoice should emit VoiceCallIncoming");
    }

    #[test]
    fn test_start_voice_rejects_text_channel() {
        let (conn, owner) = setup();
        let ch_id = channels::create_channel(&conn, "text", ChannelType::Text, None, 0).unwrap();
        let result = handle_request(
            &conn,
            &owner,
            true,
