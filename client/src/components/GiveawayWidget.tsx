...
export default function GiveawayWidget({ serverId, giveawayId, onUnavailable, refetch }: GiveawayWidgetProps) {
  ...
  return (
    <div className="giveaway-widget">
      ...
      {giveaway.status === "open" && (
        <>
          <div className="giveaway-meta">
            ends in {formatRemaining(giveaway.ends_at - nowSecs)}
          </div>
      ...
      {/* Always-rendered footer mirroring PollWidget's: entry count left, 🔗
          bottom-right — keeps ended/cancelled giveaways linkable. */}
