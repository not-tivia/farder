...
- **Checkpoint-friendly (forward-compat for windowed replication).** Verification and the authz-fold must be expressible from a *"state as of event X"* checkpoint, not **only** from genesis — so a later rung can do windowed / pruned replication (members hold a recent window + a signed checkpoint; old history archived on anchors) without a redesign. Rung 1 single-host still replays from genesis; just don't bake in a genesis-only assumption — keep the fold a **pure function of `(prior_state, event) -> new_state`** so it composes from *any* starting state. Costs nothing now; saves a retrofit later.
...
**Windowed / tiered replication (Rung 3/4 — how the mesh scales without ever-growing per-member storage).** Members replicate a **recent window** (a count- or size-budget — e.g. "last 10k messages / last N MB" — more predictable than a time window on busy servers), not all history. Cold history is archived on fewer **durable nodes** (an anchor / the owner's box): *the mesh keeps the conversation alive; the archive needs a durable home.* This rides the attachment-capability model directly — the storage hog is attachments, not text, and bytes are already separate + GC-able, so member caches **evict old attachment blobs** (kept on the anchor) while the tiny reference events stay cheap. With **no** anchor, old data simply expires from the window — which for a privacy app is a *feature* (ephemerality; ties into existing `retention_secs`). The enabling primitive is the **signed checkpoint** ("verified state as of event X" — membership/permissions/chain-heads) that lets a member trust a recent window without replaying from genesis (cf. git shallow clone / blockchain pruning); Rung 1's "checkpoint-friendly" constraint keeps the door open. Note: encrypted payloads don't compress, so "compress old messages" buys little — the lever is *eviction of old attachment bytes*, not compression of (tiny, encrypted) text.
