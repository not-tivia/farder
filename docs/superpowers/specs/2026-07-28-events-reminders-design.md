...
- **Started card:** header shows `.event-happening` "Happening now", then after the start the same block reads as a past event ("Started <local time>"); RSVP buttons `disabled`.
