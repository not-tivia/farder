...
| 19 | **Event cards (RSVP)** (shipped 2026-07-28) | **server-features-channel-only** | Row 5's rule extended: `RunCommand` kind `"event"` is rejected server-side in E2EE channels, so no event can exist there; `RsvpEvent`/`ClearRsvp`/`CancelEvent`/`EditEvent` inherit the same class-aware rejection as `VotePoll` (unreachable in practice), and the sweeper's start announcement cannot write into an E2EE channel (`insert_message*` choke point). Encrypted RSVP state is a redesign, not a port: the title, the start time and the whole attendee roster are server-side state a sealed channel exists to hide. |
| 20 | **Personal reminders (`/remind`)** (shipped 2026-07-28) | **server-features-channel-only** | `reminders.text` is stored server-side in **plaintext**, so a `/remind` set inside a sealed channel would hand the host content the channel promises to hide. Rejected at the same point as row 4 (it is a `RunCommand` kind). Nothing is posted, so there is no choke-point write to gate; `ListMyReminders`/`CancelReminder` are id/owner-keyed and channel-agnostic. The due **DM** is the row-2 class — server-encrypted with the system identity's key, which the server holds — the same honest caveat as ticker-bot alert DMs, not a new one. |
