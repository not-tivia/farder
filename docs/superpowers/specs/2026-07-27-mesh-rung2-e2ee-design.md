...
This is a product decision about the owner's own servers, so it is an **open product question** (Q8) rather than a decision made here. The two shapes:

- **(a) Migration flow** — an owner-triggered "establish the log on this server" action (genesis + owner-signed `MemberJoined`-equivalents backfilling existing members + `ChannelCreated` backfill for existing channels, all plaintext-class). Scoped as conditional sub-project 8.
- **(b) Fresh servers only** — E2EE requires a post-Rung-1 server; existing communities must be recreated to use private channels.

Whichever is chosen must be stated in the spec before sub-project 3, because it changes ingest and the fold's bootstrap rules.
...
8. **(Conditional on Q8) Existing-server migration.** Owner-triggered genesis establishment + member/channel backfill for pre-Rung-1 servers.

Order: 1 → 2 → 3 → 4 strictly; then 5, 6, 7 in any order (7 may precede 5 if the owner prioritizes at-rest protection); 8 whenever Q8 is answered (a). Each lands with its docs per the documentation-discipline checklist.
...
8. **Your existing servers cannot use private channels without a migration step.** Servers set up before the log shipped have no signed-log foundation, and long-standing members are invisible to it. Options: (a) build a one-click "turn on the log on this server" that also registers your existing members and channels; or (b) private channels only on newly-created servers, and existing communities get recreated if they want them. *Recommended default: (a) — you have real communities with real history; recreating them to get one feature is a bad trade. It is roughly one extra sub-project.*
