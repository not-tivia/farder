...
- **Partial reset (spec C7), DERIVED not latched.** The gate is
  ...
  **terminal**: ban a welcomed joiner before it confirms and the Remove-commit
  ...
  The reset generation's add-commit is never a log event, so the first
  `MlsLeafConfirmed` of a STAGED leaf seeds `reset_expected_tree_hash` and every
  later one must match it (ambiguity #7).
