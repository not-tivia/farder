...
- **Partial reset (spec C7), DERIVED not latched.** The gate is
  ...
  reset STAGED (`reset_welcomed`) is still in `leaves_pending`. Sealed sends are
  invalid throughout, so a partial reset is a **dead channel, loudly**, never a
  silent partition — but the obligation is discharged by EITHER path:
  a confirmation promotes the leaf out of `leaves_pending`, and a Remove-commit
  drops it (the bridge's own answer when a welcomed device is banned or lost
  before it confirms). The `bool` latch this replaced was set by `MlsGroupReset`
  ...
  **terminal**: ban a welcomed joiner before it confirms and the Remove-commit
  ...
  so a leaf that was dropped and later re-added is an **ordinary** pending join
  (which never seals the channel), not a resurrected reset obligation. Pinned by
