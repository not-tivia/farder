...
**Download** (`handle_download_stream`): looks up the file record, verifies the requesting member has `VIEW_CHANNEL | READ_MESSAGES` on at least one channel the file was attached to, sends a `DownloadResponse::Start` metadata frame, then streams the raw file bytes. Both the "file not found" and "access denied" cases return `DownloadResponse::Error { reason: "not available" }` — the reason string is intentionally identical so a `file_id` or `content_hash` cannot be used as an existence oracle (mesh design: existence-oracle hardening). For mesh-mode servers, the `message_attachments` join rows that the permission gate reads are materialized by `event_ingest::derive_attachments` at ingest time, so a member's attachment download access automatically follows log membership.
