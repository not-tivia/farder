...
**Download** (`handle_download_stream`): looks up the file record, verifies the requesting member has `VIEW_CHANNEL | READ_MESSAGES` on at least one channel the file was attached to, sends a `DownloadResponse::Start` metadata frame, then streams the raw file bytes.
