...
| — | `FetchDeviceCerts` / `DeviceCerts` | the device-lifecycle events for one identity — the raw signed `Event` bytes carrying `DeviceAuthorized` (one `DeviceCert` each) and, since sub-5, `DeviceRevoked` (one revoked `DeviceId` each), oldest-first and mixed. A client tells the two payloads apart by decoding each event's payload enum (no new wire shape). This is the production source of the receive-side leaf-binding gate's trust anchor (`verify_leaf_binding`), so a client must fetch the cert from HERE (the log) and **never** take one from the commit under validation. Public *within* the server by design (like `FetchKeyPackages`); membership gating still applies. Un-paginated like `FetchKeyPackages` — a cert-per-device set is tiny. A `DeviceRevoked` is matched through the identity's own device set (its `DeviceAuthorized` rows), so an owner-authored revocation is included too |
...
| `CancelEvent` | `event_id: i64` | Cancel an upcoming event. **Creator-or-MANAGE_SERVER**, timeout-gated, shares the `widget_limiter` (10/10 s). Broadcasts `EventUpdated` (`status = "cancelled"`); **no channel message** — the card flip is the record. The DMs to the Going list are sent by the sweeper's cancel-notify pass within one tick, not here. |
| `EditEvent` | `event_id: i64`, `title: String`, `description: Option<String>`, `location: Option<String>`, `starts_at: u64`, `remind_lead: Option<u64>` | **Full replace** of the editable fields (no partial-patch ambiguity), upcoming events only. **Creator-or-MANAGE_SERVER**, timeout-gated, shares the `widget_limiter` (10/10 s) — unlike cancel it is repeatable, so the limiter is its only bound against broadcast amplification. Re-runs the creation validation (title 1–120, location ≤120, description ≤500, `now+60 <= starts_at <= now+365d`, `remind_lead ∈ {900, 3600, 86400}`). A changed `starts_at` re-arms the lead reminder (`reminded_at = NULL`). Broadcasts `EventUpdated`. |
...
| `DeviceCerts` | `events: Vec<Vec<u8>>` | One identity's device-lifecycle events — `DeviceAuthorized` and `DeviceRevoked` (raw signed `Event` bytes), oldest-first and mixed — in response to `FetchDeviceCerts`. Told apart by decoding each event's payload enum. |
