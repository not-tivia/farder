...
- **`handlers.rs`** — owns ALL gates: default-deny membership, `channel_visible` (VIEW_CHANNEL / DM-participant with opaque not-found), `is_timed_out` on mutations, `widget_limiter` (`RateLimiter::new(10, 10)` on `ServerState`) for vote/retract/enter/leave and rsvp/clear/cancel-event/edit-event/cancel-reminder, MANAGE_SERVER for giveaway creation, creator-or-mod for close/cancel/reroll.
