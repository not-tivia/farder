...
**Shareable widget links (client-only — no new server surface):** every widget card carries a `.widget-copy-link` 🔗 button (PollWidget footer, GiveawayWidget meta row) that copies `farder://widget/poll/<channel_id>/<poll_id>` / `farder://widget/giveaway/<channel_id>/<giveaway_id>` to the clipboard (`navigator.clipboard` + toast, same idiom as invite links — no Tauri plugin). `Message.tsx` detects these links (`WIDGET_LINK_REGEX` / `parseWidgetLink`, ids gated by `Number.isSafeInteger`; malformed links stay plain text) and renders (a) a `.widget-link-pill` in the text (📊 "Poll link" / 🎉 "Giveaway link"; click = copy) and (b) up to 3 deduped `LinkedWidgetCard`s below the message (`.linked-widget-embeds`). Because widget links also match `INVITE_REGEX`'s `farder://` alternative, `isInviteLink` and the invite-embeds pass explicitly exclude the `farder://widget/` prefix. `LinkedWidgetCard` mounts the same interactive `PollWidget`/`GiveawayWidget`, fetching via the shipped `getPoll`/`getGiveaway` reads; a fetch failure (nonexistent id or a channel the viewer can't see — the server answers both opaquely) or a channel-id mismatch with the link renders the compact `.linked-widget-unavailable` card, so links are not an existence/content oracle. The link's `channel_id` is never sent to the server. **Refetch discipline:** `PollWidget`/`GiveawayWidget` take an optional `refetch?: "mount" | "interval"` prop — `undefined` keeps the shipped fetch-if-absent behavior, `"mount"` always fetches once on mount (fresh counts + per-viewer `my_vote`/`my_entered`), `"interval"` adds a refetch after each own interaction ack plus a 20 s interval. `LinkedWidgetCard` passes `"mount"` when the link targets the message's own channel (live `PollUpdated`/`GiveawayUpdated` events cover it) and `"interval"` otherwise (cross-channel/DM cards get no broadcasts and stay ≤20 s stale).

**Active-widgets bar:** `ActiveWidgetsBar.tsx`, rendered by `ChatPanel` between the channel header and the message list whenever a channel (including a DM) is shown. On channel switch/reconnect it calls `listActiveWidgets` and dispatches `ACTIVE_WIDGETS` (errors dispatch empty lists — the bar hides; the opaque "channel not found" is never surfaced). Renders one `.widget-chip` per open widget inside a horizontal `.active-widgets-bar` (icon + question/prize with CSS ellipsis + `.widget-chip-time` time-left re-derived on a 30 s tick — "no end" for untimed polls, "ending…" past due; chips interleave in creation order by `message_id`, the one monotonic sequence both infos carry). Clicking a chip toggles a single `.widget-chip-dropdown` anchored via `useClickAnchoredPosition`, hosting the full `PollWidget`/`GiveawayWidget` with `refetch="mount"` (live events keep the subscribed channel fresh; the mount fetch pulls per-viewer `my_vote`/`my_entered`). Outside-click/Esc closes and unmounts the dropdown; a chip removed live (close/end/cancel via `POLL_UPDATED`/`GIVEAWAY_UPDATED` reducer maintenance) leaves an already-open dropdown up until dismissed.
