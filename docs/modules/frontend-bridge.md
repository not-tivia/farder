...
| `sendSealedMessage(serverId, logServerId, channelId, content, replyTo?)` | `send_sealed_message` | Seals + submits one E2EE channel message (T10). `replyTo` is an event-hash ref (null = top-level). Returns `{ event_hash, epoch }`. Used by `MessageInput.handleSend` when the current channel is E2EE — the legacy `fetchUrl`/DM/emoji/attachment paths are excluded for this branch. |
