...
| `CancelEvent` | creator or `MANAGE_SERVER` (+ not timed out) | Upcoming only (`"event already ended or cancelled"`); `channel_events::cancel`; broadcasts `EventUpdated`. **No DMs here** — a sync arm cannot `.await`; the sweeper's cancel-notify pass DMs the Going list within one tick. |
| `EditEvent` | creator or `MANAGE_SERVER` (+ not timed out) | Upcoming only; full replace re-running the creation validation (`channel_events::validate_event_fields` + `resolve_start` + `REMIND_LEADS` membership). A changed `starts_at` passes `rearm_reminder = true`, NULLing `reminded_at`. Broadcasts `EventUpdated`. |
