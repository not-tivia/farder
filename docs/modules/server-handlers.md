...
2. **The `stale-epoch` pre-check.** An `MlsCommit` that lost the epoch CAS is an
   *accepted no-op* in the fold (Rung-3 replay determinism needs every replica to
   ...
   The same bounce fires for `MessagePostedE2ee` and `MessageEditedE2ee` whose
   `(generation, epoch)` is stale (finding F6): a sealed send that lost the
   epoch race gets the same machine-readable signal as a stale commit, because
   the remedy — fetch the winning commits, re-seal at the new epoch, resubmit —
   is identical. Without it, a stale sealed send fell through to the fold's
   prose `"sealed content does not cite the group's current epoch"`, which the
   client's exact-match `"stale-epoch"` predicate would never see. The fold's
   own `check_sealed_send` epoch check stays as the authoritative backstop; this
   is an early machine-readable bounce, not a replacement for validation.
